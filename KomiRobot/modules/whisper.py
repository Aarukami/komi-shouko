...
@bot.on(events.NewMessage(pattern="^[!?@/]izanasamabot$"))
...
async def die(event):
    ...
    dn = event.builder.article(
            ...
            description="It's a whisper Bot function for Izana!\n(c) Izana kurokawa",
            text=f"**It's a whisper bot**\n`@{me} wspr Username|Message`\n**(c) Izana kurokawa**",
            buttons=[
                [Button.switch_inline(" Go Inline ", query=" Izana ")]
                ]
            )
    await event.answer([dn])
    
@bot.on(events.InlineQuery(pattern="Izana"))
...
@bot.on(events.CallbackQuery(data="Izana"))
async def ws(event):
    user = int(db["user_id"])
    lol = [int(db["self"])]
    lol.append(user)

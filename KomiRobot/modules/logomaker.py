...
from KomiRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@komilogo", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./KomiRobot/resources/fonts/*")
    ...
    draw.text(
        ((image_width - w) / 2, (image_height - h) / 1),
        name,
        font=font,
        fill=(255, 255, 255),
    )
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Shouko komi](https://t.me/KOmixRobot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./KomiRobot/resources/fonts"):
            os.remove(font_)

...
import time
import traceback
from KomiRobot.services.keyboard import keyboard

import aiohttp
from telethon import events

from KomiRobot import telethn as Komi
from KomiRobot.modules.urluploader import download_file
from KomiRobot.utils.uputils import humanbytes, progress
...
@Komi.on(events.NewMessage(pattern="/transfersh"))
async def tsh(event):
...
@Komi.on(events.NewMessage(pattern="/tmpninja"))
async def tmp(event):
    if event.reply_to_msg_id:
        start = time.time()
        url = await event.get_reply_message()
...
@Komi.on(events.NewMessage(pattern="/up"))
async def up(event):
    if event.reply_to_msg_id:
        start = time.time()
        url = await event.get_reply_message()
        ...
        try:
            ...
            await bot.send_file(
                ...
                dosya,
                force_document=True,
                caption=f"Uploaded By *Shouko Komi*",
            )
        except Exception as e:
            traceback.print_exc()

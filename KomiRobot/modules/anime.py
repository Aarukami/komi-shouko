...
import jikanpy
import requests
from KomiRobot import DEV_USERS, OWNER_ID, DRAGONS, dispatcher
...
def airing(update: Update, context: CallbackContext):
    ...
    if len(search_str) == 1:
        update.effective_message.reply_text(
            'Format : /kairing < anime name >', parse_mode=ParseMode.MARKDOWN
        )
        return
...
def anime(update: Update, context: CallbackContext):
    ...
    if len(search) == 1:
        update.effective_message.reply_text('Format : /kanime < anime name >')
        return
    else:
        search = search[1]
...
def character(update: Update, context: CallbackContext):
    ...
    if len(search) == 1:
        update.effective_message.reply_text(
            'Format : /kcharacter < character name >')
        return
    search = search[1]
...
def user(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if message.reply_to_message:
            ...
            update.effective_message.reply_text("Format : /kuser <username>")
            return

...
from KomiRobot.modules.strings import AKI_FIRST_QUESTION, AKI_LANG_CODE, AKI_LANG_MSG, CHILDMODE_MSG, ME_MSG, START_MSG
import akinator
...
def aki_lead_cb_handler(update: Update, context:CallbackContext) -> None:
    ...
    FIND_HANDLER = CommandHandler('find', aki_find, run_async=True)
    ME_KOMI_HANDLER = CommandHandler('me', aki_me, run_async=True)
    PLAY_KOMI_HANDLER = CommandHandler('play', aki_play_cmd_handler, run_async=True)
    LANG_KOMI_HANDLER = CommandHandler('language', aki_lang, run_async=True)
    KOMI_CHILD_HANDLER = CommandHandler('childmode', aki_childmode, run_async=True)
    LEADERBOARD_HANDLER = CommandHandler('leaderboard', aki_lead, run_async=True)
    SET_LANG_KOMI_HANDLER = CallbackQueryHandler(aki_set_lang, pattern=r"aki_set_lang_", run_async=True)
    CHILD_KOMI_HANDLER = CallbackQueryHandler(aki_set_child_mode, pattern=r"c_mode_", run_async=True)    
    PLAYED_KOMI_HANDLER = CallbackQueryHandler(aki_play_callback_handler, pattern=r"aki_play_", run_async=True)    
    WIN_KOMI_HANDLER = CallbackQueryHandler(aki_win, pattern=r"aki_win_", run_async=True)
    LEAD_KOMI_HANDLER = CallbackQueryHandler(aki_lead_cb_handler, pattern=r"aki_lead_", run_async=True)
    ...
    dispatcher.add_handler(CHILD_KOMI_HANDLER)
    dispatcher.add_handler(PLAYED_KOMI_HANDLER)
    dispatcher.add_handler(WIN_KOMI_HANDLER)
    dispatcher.add_handler(LEAD_KOMI_HANDLER)
